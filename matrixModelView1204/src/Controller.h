...
//接受用户输入并传输给模型和视图, 在派生类中重写虚函数对Controller类进行覆盖
...
namespace Win
{
    ...
    public:
        ...
        virtual int hScroll(WPARAM wParam, LPARAM lParam);      // for WM_HSCROLL滚动
        ...
        virtual int lButtonDown(WPARAM state, int x, int y);    // for WM_LBUTTONDOWN: state, x, y左键按下
        virtual int lButtonUp(WPARAM state, int x, int y);      // for WM_LBUTTONUP: state, x, y左键释放
        virtual int mButtonDown(WPARAM state, int x, int y);    // for WM_MBUTTONDOWN: state, x, y中键按下
        virtual int mButtonUp(WPARAM state, int x, int y);      // for WM_MBUTTONUP: state, x, y中键释放
        virtual int mouseHover(int state, int x, int y);        // for WM_MOUSEHOVER: state, x, y鼠标在窗口中
        virtual int mouseLeave();                               // for WM_MOUSELEAVE鼠标离开窗口
        virtual int mouseMove(WPARAM state, int x, int y);      // for WM_MOUSEMOVE: state, x, y鼠标移动
        virtual int mouseWheel(int state, int d, int x, int y); // for WM_MOUSEWHEEL: state, delta, x, y滚轮
        virtual int notify(int id, LPARAM lParam);              // for WM_NOTIFY: controllerID, NMHDR
        virtual int paint();                                    // for WM_PAINT
        virtual int rButtonDown(WPARAM wParam, int x, int y);   // for WM_RBUTTONDOWN: state, x, y右键按下
        virtual int rButtonUp(WPARAM wParam, int x, int y);     // for WM_RBUTTONUP: state, x, y右键释放
